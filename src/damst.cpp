...
std::vector<unsigned> DensityAwareMST::opt() const {
    ...
    while (optimizing) {
        ...
        for (size_t i=0;i<edges.size();i++){
            // TODO I normally need to check weights with the same value and different index
            if (weights[i] > maxw and weights[i] < last_maxw){
                maxw = weights[i];
                lmi = i;
            }
        }
        ...
        // Cut the edge
        EdgeIter ei, ei_end, next;
        boost::tie(ei, ei_end) = boost::edges(*gr);
        for (next=ei; ei != ei_end; ei=next) {
            if( edges[lmi].first == boost::source(*next, *gr) and edges[lmi].second == boost::target(*next, *gr)) {
                remove_edge(boost::source(*next, *gr), boost::target(*next, *gr), *gr);
                break;
            }
        }
        ...
        for (size_t i=0; i < component.size(); i++) {
            for (size_t j=0;j<edges.size();j++) {
                if (edges[j].first == i or edges[j].second == i) {
                    subedges[component[i]].push_back(edges[j]);
        ...
        for (size_t i=0;i<num_components;i++) {
            subgraphs.push_back(Graph(&subedges[i][0], &subedges[i][0]+subedges[i].size(), &subweights[i][0], subnum_nodes[i]));
        }
    }
