...
std::vector<unsigned> DensityAwareMST::opt() const {
    ...
    // subgraphs.push_back(new Graph(*graph));
    ...
    while (optimizing) {
        ...
        for (size_t i=0;i<num_components;i++) {
            // subgraphs.push_back(new Graph(&subedges[i][0], &subedges[i][0]+subedges[i].size(), &subweights[i][0], subnum_nodes[i]));
...
double DensityAwareMST::score(const std::vector<double> w) const {
    ...
    return w.size() / (1000 * tot_weight + 1);
